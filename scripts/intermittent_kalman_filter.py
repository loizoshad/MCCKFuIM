...
class IntermittentKalmanFilter():
    def __init__(self):
    ...
    def init_ros(self):
        ...
        self.log_data_pub = rospy.Publisher('dji/log/interm_kf', Float64MultiArray, queue_size=1)
    ...
    def uwb_pos_callback(self, msg):
        ...
        self.uwb_pos[0] = msg.point.x
        self.uwb_pos[1] = msg.point.y
    ...
    def init_weights(self):
        ...
        self.R = np.diag([1.0, 1.0, 0.0000001, 0.0225, 0.0225, 0.0000001, 0.0001, 0.0001, 0.0001])
    ...
    def update(self):
        # Handling intermittent measurements
        if self.cam_available_x or self.cam_ctr_x <= 3:
            self.E[0][0] = 1
            self.cam_available_x = False; self.cam_ctr_x += 1
        else:
            self.E[0][0] = 0
        if self.cam_available_y or self.cam_ctr_y <= 3:
            self.E[1][1] = 1
            self.cam_available_y = False; self.cam_ctr_y += 1
        else:
            self.E[1][1] = 0                    

        if self.uwb_available or self.uwb_ctr <= 3:
            self.E[3][3] = 1; self.E[4][4] = 1
            self.uwb_available = False
            self.uwb_ctr += 1
        else:
            self.E[3][3] = 0; self.E[4][4] = 0
            

        self.C_aug = self.E @ self.C

        S = self.C_aug@self.P@self.C_aug.T + self.R
        K = self.P@self.C_aug.T@np.linalg.inv(S)
        self.x = self.x + K@(self.Z - self.C_aug@self.x)
        self.P = (np.eye(self.n) - K@self.C_aug)@self.P
...
if __name__ == '__main__':
    rospy.init_node('intermittent_kalman_filter', anonymous=True)
    kalman_filter = IntermittentKalmanFilter()
    kalman_filter.run()